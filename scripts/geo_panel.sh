...
echo "== 1. export ramp() from YoYHeatmap so both panels share one palette"
if grep -q '^export function ramp' "$SRC/YoYHeatmap.jsx"; then
  echo "   already exported"
else
  sed -i 's/^function ramp(v, cap = 10)/export function ramp(v, cap = 10)/' "$SRC/YoYHeatmap.jsx"
  grep -q '^export function ramp' "$SRC/YoYHeatmap.jsx" || { echo "   FAILED"; exit 1; }
  echo "   ok"
fi

echo "== 2. write GeoPanel.jsx"
cat > "$SRC/GeoPanel.jsx" <<'JSX'
...
echo "   wrote $SRC/GeoPanel.jsx"

echo "== 3. context around the YoYHeatmap mount (for wiring)"
grep -n -B10 "YoYHeatmap city" "$SRC/App.jsx"
